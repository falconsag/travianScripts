...
// @description    Shows travian resources (for Travian Legends and Travian Codex Victoria)
...
DICT = {
	en: {
		...
		servertypeh : "Server types: Travian Legends, Travian Codex Victoria",
		servertypeo : ['update','Codex Victoria','Legends'],
 ...
 ar: { // Arabic, thx ww_start_t
	...
	ro: { // Romanian language, thx bubulu
		...
		servertypeh : "Tipuri de servere: Travian Legends, Travian Codex Victoria",
		servertypeo : ['update','Codex Victoria','Legends'],
	...
	id: {
		...
		servertypeh : "Tipe Server: Travian Legends, Travian Codex Victoria",
		servertypeo : ['pembaruan','Codex Victoria','Legends'],
...
function getTTime(dist, speed, arena, artefact, shoes, leftHand) {
	...
	var abonus = ((RB.Setup[46] == 1 || RB.Setup[49] == 1) ? 0.2 : 0.1); //if Travian Codex Victoria server, or Travian 4.4 2019 version, then double the Tournament Square bonus
...
function getUserID() {
	...
	function getID() {
		ajaxRequest(fullName+'spieler.php', 'GET', null, function(ajaxResp) {
			var ad = ajaxNDIV(ajaxResp);
			var aV = $xf('//a[(@class="active") and contains(@href,"uid")]', 'f', ad);
			ad = null;
			if (aV) {
				var uId = aV.href.match(/uid=(\d+)/)[1];
...
acss += "table#vlist td{padding:0;line-height:16px;text-align:"+docDir[0]+";white-space:nowrap;}table#vlist thead td{background-color:#FFF;height:22px;text-align:center;padding:0px 3px;}" +
	...
	acss += "span."+allIDs[48]+" {position:absolute;"+(newTG?"margin-top:2px;":"margin-top:-15px;")+docDir[0]+(newTG?":122px":":130px")+" !important;width:36px !important;}"+
	...
	"ul#outOfGame {" + (newTG?"":"width:335px;") + "} div.name {white-space:nowrap}";
...
function neededResAdd () {
	...
	for( var i = 0; i < baseWrap.snapshotLength; i++ ) {
		...
		if (base.parentNode.classList.contains("contractWrapper") || base.parentNode.classList.contains("information") || base.parentNode.classList.contains("details") ) {
...
function marketSummReal () {
	...
	if( merchInWork != 0 && merchB == 1 ) {
		if( RB.dictionary[23] != '' && RB.dictionary[23] != i ) ownMer=true;
	} else {
...
// add 2x 1x tables
	if (!ownMer) if( RB.Setup[33] > 0 ) {
		...
		for (i = 0; i < aT.length; i++) {
			...
			if( ! (aT[i].nextElementSibling) ) break;
			if( aT[i].nextElementSibling.nodeName.match(/h4/i) ) break;
		}
		if( extRT.length > 0 ) {
			lastTimerB = timerB.length;
			...
			for (var i = 0; i < aT.length; i++) {
				...
				if( ! (aT[i].nextElementSibling) ) { i++; break; }
				if( aT[i].nextElementSibling.nodeName.match(/h4/i) ) { i++; break; }
			}
	...
	for (i = 0; i < aT.length; i++) {
		...
		if (ownMer == false && next == false) resourceCalculator( aT[i], timeToGo, incomingRes, 0 );
		else { showMer(aT[i], incomingRes); continue; }
		if( ! (aT[i].nextElementSibling) ) break;
		if( aT[i].nextElementSibling.nodeName.match(/h4/i) ) { next = true; }
	}
	...
	if ( vlist ) {
		...
		for ( var vn = 0; vn < villages.length; vn++ ) {
			...
			if( RB.Setup[21] != 2 && RB.Setup[15] > 0 ) {
				...
				if (newTG) if (screen.width <= 1024) linkEl.parentNode.style.height = "36px";
			}
		}
	} else {
		villages_count = 1;
...
if( RB.bodyH[0] == 1 ) {
	if( timerN.length < 4 ){
		...
		timerN[j].divme1 = $ee('div',$em('span',[timerN[j].resotime,' ',timerN[j].perreso],[['style',"white-space:nowrap;"]]),
			[['style','position:absolute;top:'+(newTG?'34px':'45px')+';left:'+rect[j]+'px;text-align:center;width:'+(newTG?'86px':'80px')+';background-color:#fef0ce;border-radius:5px;']]);
...
function distanceToMyVillages() {
	...
	var artSp = $ee('div',$em('div',[' , ',$e('img',[['class','artefact_icon_4'],['src','img/x.gif'],['title',gtext("speedart")]]),' ',sel3],[['class','artefacts'],['style','width:100%;padding:0px;']]),[['id','build'],['class','gid27'],['style','display:inline-block;']]);
...
var vLinksPat = '//div[@id="sidebarBoxVillagelist"]//li/a/div[@class="name"]';
...
function bigQuickLinks () {
	...
	function CreateBigLinkButton (strBuilding, iNo, img) {
		if (newTG) var imgC = img.cloneNode(true);
		var pos = '';
		if (iNo == 8) { if (newTG) { pos = docDir[0] + ':85px;'; imgC.style.height = '20px'; } else { pos = docDir[0] + ':160px;' } }
		if (iNo == 9) { if (newTG) { pos = docDir[0] + ':170px;'; imgC.style.height = '23px'; } else { pos = docDir[0] + ':200px;' } }
		if (newTG) {
			var BigLinkButton = $e('a',[['title',tt[iNo]],['class','layoutButton ' + strBuilding + ' green ' + (RB.village_Dorf2[bigIcon[iNo][1]] != 0 ? '' : 'disabled')],['style',pos]]);
		} else {
			var BigLinkButton = $e('button',[['type','button'],['title',tt[iNo]],['class','layoutButton ' + strBuilding + ' green ' + (RB.village_Dorf2[bigIcon[iNo][1]] != 0 ? '' : 'disabled')],['style',pos]]);
		}
		...
		//}
		if (newTG) BigLinkButton.appendChild(imgC);
		if( bigIcon[iNo][0] ) {
			if( RB.village_Dorf2[bigIcon[iNo][1]] != 0 ) {
				newTG ? BigLinkButton.href = "build.php?id="+RB.village_Dorf2[bigIcon[iNo][1]]+bigIcon[iNo][2] : BigLinkButton.addEventListener("click", function(e) { window.location.href="build.php?id="+RB.village_Dorf2[bigIcon[iNo][1]]+bigIcon[iNo][2]; }, false);
			}
		} else {
			newTG ? BigLinkButton.href = bigIcon[iNo][1] : BigLinkButton.addEventListener("click", function(e) { window.location.href=bigIcon[iNo][1]; }, false);
		}

		if (iNo == 8 || iNo == 9) { //insert plus image only if great barracks or great stable exist
			BigLinkButton.appendChild($e('img', [['class','productionBoost'],['src','img/x.gif'],['style','position:absolute;'+docDir[0]+(newTG?':20px':':3px')+';top:4px;z-index:1000']]));
		}

		if (!newTG) BigLinkButton.appendChild($e('i'));
	...
	var bigIconsFooter = newTG ? $gc('content', sidebarBoxActiveVillage)[0] : $gc('footer', sidebarBoxActiveVillage)[0];
	var childrenB = newTG ? $gt('a',bigIconsHeader) : $gt('button',bigIconsHeader);
	var imgs = [];
	if (newTG) {
...
function villageHintEdit () {
	...
	var vNF;
	newTG ? vNF = $g('villageName') : vNF = $g('villageNameField');
	...
	newTG ? vNF.parentNode.insertBefore(vNFA, vNF.nextSibling) : vNF.parentNode.appendChild(vNFA);
}
...
function scanTroopsData () {
	...
	RB.tropsI = [40,35,50,120,100,150,30,6*m,50,1,30,65,35,100,130,160,70,5*m,20,1,70,40,25,150,160,210,80,7*m,50,1,0,20,10,140,160,20,40,16*m,0,2,120,65,50,550,440,320,100,14*m,100,3,180,80,105,550,640,800,180,10*m,70,4,60,30,75,900,360,500,70,4*m,0,3,75,60,10,950,1350,600,90,3*m,0,6,50,40,30,30750,27200,45000,37500,4*m,0,5,0,80,80,4600,4200,5800,4400,5*m,3000,1,
				 ...
				 10,30,20,45,60,30,15,7*m,15,1,30,55,40,115,100,145,60,6*m,50,1,65,50,20,170,180,220,80,7*m,45,1,0,20,10,170,150,20,40,16*m,0,2,50,110,50,360,330,280,120,15*m,50,2,110,120,150,450,560,610,180,10*m,70,3,55,30,95,995,575,340,80,4*m,0,3,65,55,10,980,1510,660,100,3*m,0,6,40,50,50,34000,50000,34000,42000,4*m,0,4,0,80,80,4560,5890,4370,4180,5*m,3000,1,
...
function detectTribe () {
	var tribeImg = $xf('.//i[contains(@class,"tribe")]','f',$g('sidebarBoxHero'));
	if (tribeImg) {
...
function checkT442019 () {
	var s_nav = $g('statisticsV2');
	if( s_nav ) {
		if (RB.Setup[49] == 0) {
			RB.Setup[49] = 1;
			saveCookie( 'RBSetup', 'Setup' );
		...
		for( var i=0; i<aCCs.length; i++ ) {
			if( neFL ) {
				...
				newT.appendChild($em('TR',[
					$c((typeof aCCs[i][3].uid != "undefined" ? $em('A',[aCCs[i][0],(aCCs[i][3].v<8?$e('i',[['class','tribe'+aCCs[i][3].v+'_small']]):"")],[['href','spieler.php?uid='+aCCs[i][3].uid[0]]]):"")),
...
function returnQuickHelp () {
	var i = "return Travian.Game.iPopup(0,0);";
	if (newTG) {
		...
		$ib($ee('li',$ee('A',trImg(''),[['href','#'],['onclick',i]]),[['class',"help"]]),$g('outOfGame').firstChild);
	}
}
...
(RB.Setup[49] == 1) ? [3210,2050,2750,3830,4,6,1.40] : [1460,930,1250,1740,4,6,1.40], //Brewery Cost gid = 35
...
function TM_ShowMainBuildingNumbers(){
	...
	if ( mapInfo ) {
		...
		var imageElements = $xf('.//img[contains(@class,"building") or contains(@class,"wall")]','l',mapInfo);
	...
	for (var i = 0; i < countArray; i++) {
		if (dorf == 1 ) {
			...
			BuildingLevel = parseInt(BuildingLevel[0]);

			if (i == 0) { var typeOfVillage = $g('village_map').getAttribute('class').split(' '); }
			gid = fieldsOfVillage[typeOfVillage[0]][i]+1;
			aid = i+mapOffset;
		...
		if (blswitch && (dorf == 2)) { smallDIV.style.borderRadius = "50%"; smallDIV.style.backgroundImage = "none"; }
...
function getMaxLevel(gid) {
	...
	switch (gid) {
		...
		case 23:
			...
			break;
		case 35:
			RB.Setup[49] ? maxLevel = 20 : maxLevel = 10;
			break;
		case 40:
			maxLevel = 100;
			break;
...
function reportsDelOrSearch () {
	...
	newDel.setAttribute('title',delBtn[0].textContent);
	...
	if ($g("mark_as_read")) {
		var newMarkAbove = $g("mark_as_read").cloneNode(true);
		delBtn[0].parentNode.insertBefore(newMarkAbove, $g("sAll1").parentNode.parentNode);
	}
	var newDelAbove = delBtn[0].cloneNode(true);
	ltr ? newDelAbove.style.marginLeft = "4px" : newDelAbove.style.marginRight = "4px";
	delBtn[0].parentNode.insertBefore(newDelAbove, $g("sAll1").parentNode.parentNode);
	if ( archiveButton ) {
		var newArcAbove = archiveButton.cloneNode(true);
		ltr ? newArcAbove.style.marginLeft = "4px" : newArcAbove.style.marginRight = "4px";
		delBtn[0].parentNode.insertBefore(newArcAbove, $g("sAll1").parentNode.parentNode);
	}
	var newDelAllAbove = delBtn[0].cloneNode(true);
	newDelAllAbove.textContent = " [X] >> ";
	delBtn[0].parentNode.insertBefore(newDelAllAbove, $g("sAll1").parentNode.parentNode);
...
function displayWhatIsNew () {
	if ($g('whatsnew')) {
		...
		content.innerHTML = "What's new in Version "+version+" - Oct 28, 2019:<p></p><ui><li>Minor bugfixes</li><li>Added Indonesian language</li></ui>";
...
// start script
	...
	var idx = [1,2,3,5];
	var rect = ltr ? [94,178,262,439] : [430,346,262,85];
	if (newTG) { rect = ltr ? [94,183,272,460] : [458,369,280,92] }
	...
	if( /berichte.php.+id=/.test(crtPath) ) { addSpeedAndRTSend(); analyzerBattle(); getTroopNames(); }
	...
	if ( /statistiken.php\?id=8/.test(crtPath) ) checkT442019();
